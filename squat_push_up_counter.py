...
while cap.isOpened():
    ...
    if len(landmark_list) != 0:
        # Right-side angles (already defined)
        elbow_angle = get_angle(img, landmark_list, 12, 14, 16)
        shoulder_angle = get_angle(img, landmark_list, 14, 12, 24)
        hip_angle = get_angle(img, landmark_list, 12, 24, 26)

        # Left-side angles (added these calculations)
        elbow_angle_left = get_angle(img, landmark_list, 11, 13, 15)
        shoulder_angle_left = get_angle(img, landmark_list, 13, 11, 23)
        hip_angle_left = get_angle(img, landmark_list, 11, 23, 25)

        # Calculating push-up success percentage based on right elbow angle
        pushup_success_percentage = np.interp(elbow_angle, (90, 160), (0, 100))
        pushup_progress_bar = np.interp(elbow_angle, (90, 160), (380, 50))

        # Is the form correct at the start? Check both sides
        if (
            elbow_angle > 160
            and shoulder_angle > 40
            and hip_angle > 160
            and elbow_angle_left > 160
            and shoulder_angle_left > 40
            and hip_angle_left > 160
        ):
            form = 1

        # Full push-up motion (checking for both sides)
        if form == 1:
            if pushup_success_percentage == 0:
                if (
                    elbow_angle <= 90
                    and hip_angle > 160
                    and elbow_angle_left <= 90
                    and hip_angle_left > 160
                ):
                    feedback = "Go Up"
                    if direction == 0:
        ...
        # Show the push-up count
        cv2.rectangle(img, (0, 380), (100, 480), (0, 255, 0), cv2.FILLED)
        cv2.putText(img, str(int(count)), (25, 455), cv2.FONT_HERSHEY_PLAIN, 5,
                    (255, 0, 0), 5)

        # Show the push-up feedback
